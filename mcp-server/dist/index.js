...
const THEMES = ["light", "dark", "chatgpt", "ocean", "sunset"];
function getApiUrl() {
    ...
    tools: [
        {
            ...
            description: "Render markdown to a PNG image without storing. Best for one-off chat shares (WhatsApp, iMessage). Returns base64 PNG.",
            inputSchema: {
                type: "object",
                properties: {
                    content: {
        ...
        },
        {
            ...
            description: "Store markdown and return a shareable URL with OG preview. Best for Discord, Slack, and persistent links.",
            inputSchema: {
                type: "object",
                properties: {
                    content: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "render_markdown_to_image") {
            const data = await postJson("/api/render?format=base64", { content, title, theme });
            return {
                content: [
                    {
                        type: "text",
        ...
        if (name === "create_embed_link") {
            const data = await postJson("/api/create", { content, title, theme });
            return {
                content: [
                    {
                        type: "text",
